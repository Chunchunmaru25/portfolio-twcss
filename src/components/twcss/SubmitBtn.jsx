...
const Button = () => {
    ...
    const sendLetters = "Send Message".split("");
    const sentLetters = "Sent".split("");

    return (
        <>
            {/* Custom CSS for all complex animations (Tailwind can't handle conic gradients, multi-keyframe states, etc.) */}
            <style>{`
                .button {
                    --primary: #ff5569;
                    --neutral-1: #f7f8f7;
                    ...
                    font-size: 18px;
                    font-weight: 600;
                    background: linear-gradient(to bottom, #2a2a3a, #1f1f2e);
                    color: white;
                    overflow: hidden;
                }
                ...
                /* Tailwind-friendly base + custom fancy effects */
                .outline {
                    position: absolute;
                ...
                /* Sent state (triggered by React state instead of :focus) */
                .sent .state--default { position: absolute; opacity: 0; }
                .sent .state--sent { display: flex; }
                .sent .state--sent span {
                    opacity: 0;
                    animation: slideDown 0.8s ease forwards calc(var(--i) * 0.2s);
                }
                .sent .state--sent .icon svg {
                    opacity: 0;
                ...
                .sent .state--default svg { animation: takeOff 0.8s linear forwards; }
                .sent .state--default .icon { transform: rotate(0) scale(1.25); }
                ...
                .sent .state--default .icon:before {
